...
class TSception(nn.Module):
    def conv_block(self, in_chan, out_chan, kernel, step, pool):
        ...
        if pool > 1:
            layers.append(nn.AvgPool2d(kernel_size=(1, pool), stride=(1, pool)))
        return nn.Sequential(*layers)

    def __init__(self, num_classes, input_size, sampling_rate, num_T, num_S, hidden, dropout_rate):
        # input_size: 1 x EEG channel x datapoint
        super(TSception, self).__init__()
        self.inception_window = [0.125]
        self.pool = max(1, min(8, int(input_size[-1]) // 16))
        ...
        spatial_pool = max(1, int(self.pool * 0.25))
        fusion_pool = max(1, self.pool // 2)
        self.Sception1 = self.conv_block(num_T, num_S, (int(input_size[1]), 1), 1, spatial_pool)
        self.Sception2 = self.conv_block(num_T, num_S, (int(input_size[1] * 0.5), 1), (int(input_size[1] * 0.5), 1),
                                         spatial_pool)
        self.fusion_layer = self.conv_block(num_S, num_S, (3, 1), 1, fusion_pool)
